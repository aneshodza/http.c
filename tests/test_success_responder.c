...
void test_success_response() {
  ...
  char *response = success_response(path, 1, NONE, NULL);
...
void test_success_response_no_body() {
  char *path = "./public_test/index.html";
  char *response = success_response(path, 0, NONE, NULL);
  CU_ASSERT_STRING_EQUAL(response, "HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: 30\r\nConnection: close\r\n\r\n");
}
...
void setup_test_success_responder(CU_pSuite *suite) {
  ...
  CU_add_test(*suite, "Success responses can be sent without a body", test_success_response_no_body);
}
